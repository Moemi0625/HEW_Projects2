...
function AddStats() {
 ...
 p[0].innerHTML = `Time: ${minutes} Mins and ${seconds} Sec`;
// スタイルを変更
p[0].style.fontSize = '15px';  
p[0].style.fontWeight = 'lighter';


// Star Rating
p[1].innerHTML = `Star Rating: ${starCount}/5`;
// スタイルを変更
p[1].style.fontSize = '25px';  
p[1].style.marginTop = '-20px';
}
...
function updateReflection(card, degree, percentage) {
  card.style.background = `linear-gradient(${degree}deg, rgba(51, 23, 31, 0.7) 0%,rgb(48, 47, 67, 0.6) ${percentage}%,rgb(48, 47, 67, 0.8) 100%)`;
